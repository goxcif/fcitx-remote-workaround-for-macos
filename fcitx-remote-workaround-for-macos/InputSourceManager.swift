...
class InputSource: Equatable {
    ...
    static func isProperInputSource(_ source: TISInputSource) -> Bool {
        guard let category: String = getProperty(source, kTISPropertyInputSourceCategory)
            , let selectable: Bool = getProperty(source, kTISPropertyInputSourceIsSelectCapable) else
        {
            return false
        }
    ...
    convenience init() {
        self.init(tisInputSource: TISCopyCurrentKeyboardInputSource().takeUnretainedValue())
    }
